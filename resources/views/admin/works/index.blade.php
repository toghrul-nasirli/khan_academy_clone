                                ...
                                <tbody>
                                    @foreach ($works as $work)
                                        <tr style="line-height: 80px;">
                                            ...
                                            <td>{{ $work->title }}</td>
                                            <td>
                                                <a href="{{ route('admin.works.edit', $work) }}" class="px-1"><i class="fas fa-edit"></i></a>
                                                <a href="javascript:void(0)" class="delete-btn px-1" data-id="{{ $work->id }}"><i class="fas fa-trash-alt"></i></a>
                                                <form id="delete-form{{ $work->id }}" action="{{ route('admin.works.destroy', $work) }}" method="POST" class="d-none">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                            </td>
